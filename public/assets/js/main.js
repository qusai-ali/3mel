...
(function ($) {
	...
	$(window).bind("load", function () {
		if ($("body:has(#preloader)").length > 0) {
	...
	$('.popup-btn').on('click',function(){
		...
		console.log(popup_val);
		$('.popup-box').each(function(){
			if($(this).hasClass(popup_val)) {
				$(this).addClass('open');
			} else {
	...
	$('.popup-box .pop-overlay').on('click',function(){
		$(this).parent().removeClass('open');
	});
	$('.login-popup .titles-list li').on('click',function(){
		if(!$(this).hasClass('active')) {
			var form_title = $(this).data('value');
			$(this).addClass('active');
			$(this).siblings().removeClass('active');
			$('.login-popup form').each(function(){
				if ($(this).hasClass(form_title)) {
	...
	if ($(window).innerWidth() <= 767) {
		var img_height = $('.screen img').innerHeight();
		var img_width = $('.screen img').innerWidth();
		var iframe_height = img_height * 0.626302;
		var iframe_width = img_height * 0.9874; 
		$('.screen iframe').css({
			'height' : iframe_height + 'px',
			'width' : iframe_width + 'px'
		});
		$('.screen.small iframe').css({
			'height' : iframe_height + 'px',
			'width' : iframe_width + 'px'
		});
		$('.screen').css({
		...
		$(window).on('resize', function(){
			img_height = $('.screen img').innerHeight();
			img_width = $('.screen img').innerWidth();
			iframe_height = img_height * 0.626302;
			iframe_width = img_height * 0.9874; 
			$('.screen iframe').css({
				'height' : iframe_height + 'px',
				'width' : iframe_width + 'px'
			});
